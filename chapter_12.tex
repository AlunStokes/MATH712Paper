...
\begin{definition}[Cell Decomposition \emph{of $M$}]
  A \emph{\cd} of $M$ is a finite set defined by some finite subsequence $\aoneak$ (strictly increasing) that form the set
  \begin{align*}
...
\begin{definition}[Compatibility]
  We call a \cd $\fancyD$ of $M^n$ \emph{compatible} with a subset, $X \subseteq M^n$ if for each cell, $C \in \fancyD$. either $C \cap \fancyD$ is empty, or $C$ is a subset of $X$.
\end{definition}
...
\begin{theorem}[\CD]
 ...
 Then
  \begin{enumerate}[label={}]
    \item[$(I)_n$ ] Suppose $\vonevm{X}{k} \subseteq M^n$ are definable sets. Then there is a \cd of $M^n$ \cmble with each $X_j$.
    \item[$(II)_n$ ] If $\funcdom{f}{X}{M}$ is definable, then there is a \cd, $\fancyD$ of $M^n$ \cmble with $X$ s.t. the restriction $\funcrestr{f}{C}$ is \cont for each $C \in \fancyD$.
  \end{enumerate}

  Further, and in analogy to the monotonicity theorem, if our $\vonevm{X}{k}$ or $f$ (depending on case $(I)$ or $(II)$) are definable over $A \subset M$, then we can take the cells in  $\fancyD$ to be similarly definable over $A$; that is, with the \emph{same} parameters.

  \begin{svgraybox}
    This last point is perhaps a bit unfair to mention, as we will not be providing a proof for it -- though for the sake of interest, it would feel incomplete to not at least analogize with Theorem \ref{thm:monotonicity}. In truth, what follows is not a full proof of \CD, but a special case where we take $M$ to be $\R$, and use the yet unproven (or even stated) result of \emph{uniform finiteness}. We take this result entirely for granted in the lectures, due to the oddity that a complete (unassuming) proof somewhat `bootstraps' uniform finiteness into the induction we do on $(I)_j$ and $(II)_j$, proving it as we go along. For our purposes, we take it as assumedly true -- in part due to the length of this proof even \emph{with} that assumption -- and trust that our dear intelligent reader sees plainly how we could fix this in absence of assumption.
  \end{svgraybox}

\end{theorem}

Uniform finiteness is a generalization of the finiteness theorem we proved earlier (Theorem \ref{thm:2finiteness}), but with potentially many parameters and in higher dimension. As with the argument for the \CD Theorem, we will similarly restrict our attention to the case where $M = \R$. This special-case is as follows:

\begin{proposition}[Uniform Finiteness (for $\R$)]
  \label{prop:unif-finiteness}
  Suppose $X \subset \R^{n+1}$ is \defn with each fibre $X_x$ finite for $x \in \R^n$. Then there is some $N \in \Zgeq{1}$ such that $\card{X_x} \leq N$ for all $x \in \R^n$.
\end{proposition}


The following proof is due to Van Den Dries \cite{dries_tame_1998}
