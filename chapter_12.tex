...
\begin{proof}[\CD (Theorem \ref{thm:cell-decomposition})]
  ...
  to each half of the middle cup, we can apply $\In$ to determine the contents of each of the respective \emph{big} cups toe be a finite union of cells -- and so $B$ must be an \emph{countable} union of cells, each of which is contained in one of these sets. Perhaps coming a bit out of left field, we apply the Baire Category Theorem to conclude that by openness of $B$, so too must be one of these cells be open.
  \begin{svgraybox}
  ...
  %Supposing otherwise, we can cell decompose that box to get an open cell on which $f$ is constant, and so clearly \cont.


  %That is, we have a set of definable $\vonevm{X}{k} \subseteq M^n$, and we wish to show that there is some \cd of $M^n$ compatible with each $X_j$.
\end{proof}
